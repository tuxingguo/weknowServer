...
<!--
 * 李海旺
 * 区域管理模块前台浏览投诉建议前台文件，参照admins/main.jsp完成
 -->
<html>
<head>
...
<script src="<%=request.getContextPath()%>/jquery/jquery-1.7.2.min.js"
	type="text/javascript"></script>
<link rel="stylesheet" type="text/css"
	href="<%=request.getContextPath()%>/css/main.css">
<script src="<%=request.getContextPath()%>/bootstrap/bootstrap.js"
	type="text/javascript"></script>
<script src="<%=request.getContextPath()%>/js/main.js"
	type="text/javascript"></script>
<script src="<%=request.getContextPath()%>/js/domains/main.js"
	type="text/javascript"></script>
</head>
<body>
	<div class="navbar">
			...
			<div class="row-fluid">
						...
						<tbody>
							<c:forEach items="${pageResult.dataList}" var="data"
								...
								<tr>
									...
									<td>${data.id}</td>
									<td>${data.parentid}</td>
									<c:forEach items="${parentNames}" var="name" begin="${count.index}" end="${count.index}">
									<td>${name}</td>
									</c:forEach>
									<td><a
										href="<%=request.getContextPath()%>/domains/edit.action?id=${data.id}"><i
				...
				<div class="pagination">
					...
					<ul>
						...
						<li><a
							${pageResult.pageNo<=1?"":"href='"}<%=request.getContextPath()%>${"/domains/main.action?pageNo="}

							${pageResult.pageNo-1} ${"'"}>上一页</a></li>
						<li><a href="#" id="pageInfo">${pageResult.pageNo}/${pageResult.pages}</a></li>
						<li><a
							${pageResult.pageNo>=pageResult.pages?"":"href='"}<%=request.getContextPath()%>${"/domains/main.action?pageNo="}
